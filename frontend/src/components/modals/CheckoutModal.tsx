...
export const CheckoutModal = ({ isOpen, onClose, onFinish, t }: CheckoutModalProps) => {
  const { cart, activeVoucherCode, setActiveVoucher, collectedVouchers, pointPercentage } =
    useAppStore();
  ...
  // Find the active voucher from store-persisted claimed vouchers
  const activeVoucher = collectedVouchers.find((v) => v.code === activeVoucherCode);
  const discount = activeVoucher ? (activeVoucher.discount_value ?? 0) : 0;
        ...
        >
          ...
          <div className="space-y-4 mb-8">
            {cart.map((item) => (
              <div
                key={item.id}
                className="flex justify-between items-center bg-theme-bg-secondary p-4 rounded-2xl border border-theme-border"
              >
                <div>
              ...
              {collectedVouchers.length > 0 ? (
                collectedVouchers.map((v) => (
                  <button
                    key={v.code}
                    onClick={() =>
                      setActiveVoucher(activeVoucherCode === v.code ? null : v.code)
                    }
                    className={cn(
          ...
          <div className="glass p-6 rounded-[2rem] space-y-3 mb-8 border-cobalt-blue/10">
            ...
            <div className="flex justify-between items-end">
              <div>
                <p className="text-[10px] font-black uppercase text-slate-500 mb-0.5">
                  {t.total}
                </p>
                <p className="text-2xl font-black text-cobalt-blue">
          ...
          <Button
            variant="neon"
            onClick={() =>
              onFinish({ subtotal, discount, total, pointsEarned, cart, activeVoucherCode })
            }
