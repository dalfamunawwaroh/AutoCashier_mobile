...
export default function App() {
  ...
  // Handle password-reset deep link on first load
  useEffect(() => {
  ...
  // Brief splash screen, then redirect if already logged in
  useEffect(() => {
  ...
  const handleCheckoutFinish = async (details: any) => {
  ...
  if (isInitializing) {
    return (
      <div className="min-h-screen bg-theme-bg flex flex-col items-center justify-center p-8">
        <BrandLogo size={60} />
        <p className="mt-8 text-[10px] text-slate-500 font-medium tracking-[0.4em] uppercase">
          Initializing...
        </p>
      </div>
    );
      ...
      <AnimatePresence mode="wait">
        {!isLoggedIn ? (
          ...
          <div className="safe-h-screen w-full flex flex-col overflow-hidden">
                ...
                <nav className="p-6 pt-0 absolute bottom-0 left-0 w-full z-50 pointer-events-none">
                  <div className="glass p-2 rounded-[2rem] flex items-center justify-between border-theme-border backdrop-blur-3xl shadow-2xl pointer-events-auto max-w-sm mx-auto">
                    <button
                      ...
                      className={cn(
                        'flex-1 p-3 rounded-[1.25rem] flex flex-col items-center gap-1 transition-all',
                        activeTab === 'home'
                          ? 'bg-cobalt-blue/10 text-cobalt-blue font-semibold'
                          : 'text-slate-400'
                      )}
                    >
                      <Home size={18} />
            ...
            {screen === 'vouchers' && (
              <VoucherCenterScreen key="vouchers" t={t} onBack={() => setScreen('main')} />
            )}
            {screen === 'points_history' && (
              <PointsHistoryScreen key="points" t={t} onBack={() => setScreen('main')} />
            )}
            {screen === 'notifications' && (
              <NotificationsScreen key="notifications" t={t} onBack={() => setScreen('main')} />
            )}
          </div>
        )}
      </AnimatePresence>
